    ...
    <body>
       	<div class="mainBody">
			<c:if test="${not empty personList}">
				<p>GET request result (getPersonAll) from the endpoint<br>${personsURL}:</p>
			...
			<c:if test="${not empty person}">
				<p>GET request result (getPersonUnique) from the endpoint<br>${personsURL}/{id}:</p>
				<div class="servicesList">
					<div class="tableRow"><div class="tableCell">PERSON_ID</div><div class="tableCell">PERSON_NAME</div></div>
					<div class="tableRow"><div class="tableCell">${person.id}</div><div class="tableCell">${person.name}</div></div>
				</div>
			</c:if>
			<c:if test="${empty personList and empty person}">
				<p>GET request result (getPersonAll/getPersonUnique) from the endpoint<br>${personsURL}:</p>
				<p>Empty Database or specific registry not found ! NotFoundException thrown/caught !</p>
