...
public class SOAP_WS1 {
	public static void main(String[] args) {
		SpringApplication.run(SOAP_WS1.class, args);
	}
}
...
The contract for the stub object classes and the generated XML between the two servers are the WSDLs (xml/xsd files), 
one at each server. The first WSDL (at the provider) is manually created - top bottom approach, the second one (at the 
consumer) is based on the first and is automatically created by the Apache CXF API maven plugin, as the stubs. The
...
stub classes accordingly, so that the WSDL files can be generated as well as the java objects can be converted
...
server is implemented and deployed, then the consumer server is created to communicate with the provider.
...
structures needed for the XML messages and for all stub java objects later generated. The WSDL is an xml/xsd file 
that comprises basically the types (xsd), elements, messages, portType and service data, used by that server and xml APIs. 
...
needed to process future requests from the consumer. And lastly, finishing the provider server implementation, 
...
but an extended one, generated by the CXF, and added manually to the consumer server project, which maps to the 
...
provider webServices and have the responses returned and shown to the final user, with the use of JAX-WS and JAXB
behind the scenes (at the stubs automatically generated by CXF from the WSDL). At a bottom up design, on the other
hand, these two APIs would have to be used manually at the manager and dto classes of the business layer (see 
SOAP_WS2/SOAP_WS2_Client).
...
at the Constructor of the portType implementation class (provider server project). Orders are organized as
...
and of the Provider/Consumer servers, using Spring Boot, Apache CXF, JAX-WS and JAXB, having a customized and manually 
created WSDL (xsd) as a contract for the XML messages exchanges and stubs generation on each side.

For running these 2 SOAP webServices apps, run both main Spring Boot classes of each project as a java Application, 
then use a browser, on any device, to test it using the same network and local IP. Sample operation requests have 
been set at the html links of each project web UI, others can be tested using SoapUI. At the consumer side, a console 
testing class has also been created, independent from the web view UI.
